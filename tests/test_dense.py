...
@pytest.fixture
def input_tensor():
    return cp.array([[1.0, 2.0, 3.0, 4.0]])
...
def test_forward_computation_consistency(dense_layer, input_tensor):
    expected = cp.dot(input_tensor, dense_layer.w) + dense_layer.b
    actual = dense_layer.forward(input_tensor)
    cp.testing.assert_allclose(actual, expected, rtol=1e-5, atol=1e-8)

def test_backward_output_shape(dense_layer, input_tensor):
    dense_layer.forward(input_tensor)
    grad_output = cp.ones((1, 3))
...
def test_backward_gradient_values(dense_layer, input_tensor):
    dense_layer.forward(input_tensor)
    grad_output = cp.array([[1.0, 2.0, 3.0]])
    
    grad_input = dense_layer.backward(grad_output)

    expected_dw = cp.dot(input_tensor.T, grad_output)
    expected_db = cp.sum(grad_output, axis=0)

    cp.testing.assert_allclose(dense_layer.dw, expected_dw, rtol=1e-5, atol=1e-8)
    cp.testing.assert_allclose(dense_layer.db, expected_db, rtol=1e-5, atol=1e-8)
