...
def im2col_loop(X, kernel_size:int, stride=1, padding=0):
    ...
    for h in range(out_height):
        for w in range(out_width):
            ...
            col[:, :, h * out_width + w] = X_padded[:, :, h_start:h_end, w_start:w_end].reshape(N, C * kernel_size * kernel_size)
...
def test_im2col(batch_size):
    ...
    print(f"[LOOPS]Batch {batch_size}: Time = {end - start:.4f}s, Memory = {mem:.2f} MB")
...
def test__opt_im2col(batch_size):
    ...
    mem = cp.get_default_memory_pool().used_bytes() / (1024 ** 2)  # Convert bytes to MB
    print(f"[As_STRIDED] Batch {batch_size}: Time = {end - start:.4f}s, Memory = {mem:.2f} MB")
...
def col2im(col, input_shape, kernel_size, stride=1, padding=0):
    ...
    col_reshaped = col.reshape(N, H_out * W_out, C, K * K).transpose(0, 2, 1, 3).reshape(shape)
