...
"""
class Conv2D(Layer):
    def __init__(self, input_channels: int, output_channels: int, kernel_size: int, stride: int = 1, padding: int = 1):
        self.input_channels = input_channels
        self.output_channels = output_channels
        self.kernel_size = kernel_size
        self.stride = stride
        self.padding = padding
        self.w = cp.random.randn(output_channels, input_channels, kernel_size, kernel_size) * 0.01  # Initialize weights
    ...
    def backward(self, grad_output):
        ...
        x_col = im2col(self.x, self.kernel_size, self.stride, self.padding)  # shape: (N * H_out * W_out, C_in * K * K)
        ...
        dx_col = cp.dot(grad_output_reshaped, w_col)  # shape: (N * H_out * W_out, C_in * K * K)
